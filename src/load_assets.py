...
class Loader():
    ...
    def load_graphics(self):
        ...
        img_files = [os.path.join(self.sprite_folder, f) for f in files if f[-3:] == 'png']

        knight_imgs = sorted(list(filter(lambda x: 'knight' in x, img_files)))
        elf_imgs = sorted(list(filter(lambda x: 'elf' in x, img_files)))
        tileset_img = os.path.join(self.tileset_folder, '0x72_DungeonTilesetII_v1.3.png')

        gfx_lib = {
                'knight': [pg.image.load(im).convert_alpha() for im in knight_imgs],
